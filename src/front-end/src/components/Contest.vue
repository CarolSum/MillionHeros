...
import { baseInfoWrapper, allInfoWrapper } from '../utils/ObjectWrapper';
	...
	async beforeMount () {
		try {
			...
			if(res.isFinished) {
				...
				// 时间戳转为Date
				res.ddl = new Date(res.ddl);
				this.standard_answer = JSON.parse(res.answer.replace(/\'/g,"\""));
				this.content = JSON.parse(res.content.replace(/\'/g,"\""));
				this.allContestInfo = res;
			}
